...
async function main() {
    ...
    // Update header row to add missing columns
    console.log('\nUpdating users sheet headers...');
    await usersSheet.setHeaderRow(['email', 'password', 'name', 'role', 'status', 'created_at']);
    console.log('Headers updated!');

    // Clear existing rows and add new users
    const existingRows = await usersSheet.getRows();
    console.log(`\nClearing ${existingRows.length} existing rows...`);
    ...
    // Show final result
    console.log('\n=== Final Users List ===');
    const finalRows = await usersSheet.getRows();
